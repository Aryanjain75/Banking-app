document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchTransactions = async () => {
        try {
            const response = await fetch("http://localhost:5000/transection/alltransaction/", {
                method: 'GET',
                headers: {
...
transactionDetailsDiv.appendChild(table);
    ...
    const makeTransaction = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                alert('Transaction failed due to low balance  if your balance is above the limit please contact bank');
            }
            if(response.ok){
